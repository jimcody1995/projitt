...
import { generateWebRTCToken } from "@/api/meeting";
    ...
    const generateToken = useCallback(async () => {
        ...
        try {
            ...
            if (useMockData) {
                ...
                // Generate mock token (base64 encoded JSON)
                const mockTokenData = {
                    sub: 10, // User ID
                    name: "Test User",
                ...
                response = {
                    token: btoa(JSON.stringify(mockTokenData)), // Base64 encode
                    expires_at: new Date(Date.now() + 3600000).toISOString(), // 1 hour from now
                    user_id: 10,
                };

                console.log('Mock WebRTC Token Generated:', response);
                console.log('Decoded Token Data:', JSON.parse(atob(response.token)));
            } else {
                response = await generateWebRTCToken();
                console.log('WebRTC Token Generated:', response);
            ...
            setWebrtcToken(response.token);
            setTokenExpiresAt(response.expires_at || null);

            // Store token in environment/localStorage for later use
            if (typeof window !== 'undefined') {
                localStorage.setItem('video_token', response.token);
                console.log('Token saved to localStorage as video_token');
            }
            ...
            console.error('Error generating WebRTC token:', err);
        } finally {
            setIsGeneratingToken(false);
        }
    }, [useMockData]);

    // Log meeting ID when component mounts and generate token
    useEffect(() => {
        if (meetingId) {
            console.log('Meeting ID from URL:', meetingId);
            // Generate WebRTC token before joining video call
            generateToken();
            // You can use meetingId here to:
            // - Fetch meeting details
            // - Join the meeting presence
            // - Initialize WebRTC connections with the token
            // - Load meeting participants
        } else {
            console.warn('No meeting ID in URL');
        }
