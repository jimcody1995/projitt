...
export default function Interviews() {
    ...
    return (
        <div data-testid="interviews-container">
            <div className="flex justify-between w-full">
                <p className="text-[24px]/[30px] font-semibold text-[#1C1C1C]" data-testid="interviews-page-title" id="interviews-page-title">Interviews</p>
                <div className="flex items-center bg-[#e9e9e9] rounded-[6px] p-[2px]" data-testid="view-switcher" id="view-switcher">
                    <div
                        className={`w-[131px] h-[42px] rounded-[6px] flex items-center justify-center cursor-pointer ${activeTab === 'calendar' ? 'bg-[#0d978b] text-white' : 'text-[#a5a5a5]'}`}
                        onClick={() => setActiveTab('calendar')}
                        data-testid="calendar-tab-button"
                        id="calendar-tab-button"
                    >
                        <p className="text-[14px]/[22px] font-medium " data-testid="calendar-tab-text">Calendar</p>
                    </div>
                    <div
                        className={`w-[131px] h-[42px] rounded-[6px] flex items-center justify-center cursor-pointer ${activeTab === 'table' ? 'bg-[#0d978b] text-white' : 'text-[#a5a5a5]'}`}
                        onClick={() => setActiveTab('table')}
                        data-testid="table-tab-button"
                        id="table-tab-button"
                    >
                        <p className="text-[14px]/[22px] font-medium " data-testid="table-tab-text">Table</p>
                    </div>
                </div>
            </div>
